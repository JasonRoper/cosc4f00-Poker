...
public class GameService {
    ...
    /**
     * This method takes the message from the Controller when an action is received, it takes that action, interprets it, gets the gamestate and calls appropriate methods
     ...
     */
    public GameUpdateTransport bet (GameState gameState, GameAction action, long userID){
        Player player=null;
        ...
        if (player.getCashOnHand()>=action.getBet()){
            ...
            applyBet(player.getGameID(),action.getBet()); // apply the bet to gamestate
            gameState.setLastBet(player.getGameID()); // update who bet last
        } else {
    ...
    public void applyBet(int playerGameID, int bet) {
