...
class GameMechanics {
  ...
  constructor (gameId: number) {
    ...
    this.GAME_SERVICE.onGameUpdated(this.setGameState)
  }
 ...
 *//*/
  ...
  public setGameState (GameState: GameState) {
    ...
    if (GameState.GameId === this.GAME_TRANSPORT.GameId) {
      this.GAME_TRANSPORT = GameState
      this.validatePreMove()
    } else {
    
      let error = new GameError("Attempted to Replace GameState with a different GameId")
      if (error instanceof GameError) {
        console.log(error.gameError())
      }
    //throw new GameStateEVENT('Attempted to Replace GameState with a different GameId')
    }
  }
