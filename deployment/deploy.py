...
def copy_frontend_to_backend():
    ...
    shutil.rmtree("api/src/main/resources/public")
    shutil.copytree("client/dist", "api/src/main/resources/public")

def build_backend():
    print("Building backend...")
    with cd("api"):
        subprocess.run(["gradlew", "bootRepackage"], check=True, shell=True)
...
def main():
    parser = argparse.ArgumentParser(description="build a elastic beanstalk package for pokerpals")
    parser.add_argument("type", choices=["major", "minor", "patch"], help="the type is the type of upgrade that took place")
